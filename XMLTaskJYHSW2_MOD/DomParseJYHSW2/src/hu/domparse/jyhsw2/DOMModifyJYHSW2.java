...
public class DOMModifyJYHSW2 {
	...
	static String filePath = "C:\\Users\\Ede\\eclipse-workspace\\DomParseJYHSW2\\XMLJYHSW2.xml";
	static File inputFile = new File("XMLJYHSW2.xml");
	...
	public static void Vezerigazgato() {
	     try {
		...
		Node students = 
               document.getElementsByTagName("Vezerigazgato").item(0);
		
		System.out.println("Adja meg az uj nevet: ");
		String nev = sc.nextLine();
		System.out.println("Adja meg az uj Fizetest: ");
		String kereset = sc.nextLine();
		System.out.println("Adja meg az uj Szuletesi Datumot: ");
		String szul = sc.nextLine();
		
		NamedNodeMap attr = students.getAttributes();
		Node nodeAttr = attr.getNamedItem("VID");
		System.out.println("Adja meg az ID-t: ");
		int kulcs = sc.nextInt();
		...
		NodeList nList = document.getElementsByTagName("Vezerigazgato");
		for (int i = 0;i < nList.getLength();i++){
			Node nNode = nList.item(i);
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				 Element Element = (Element) nNode;
				if (i+1 == kulcs) {
					Node node1 = Element.getElementsByTagName("nev").item(0);
					node1.setTextContent(nev);
					Node node2 = Element.getElementsByTagName("fizettseg").item(0);
					node2.setTextContent(kereset);
					Node node3 = Element.getElementsByTagName("szuletesi_datum").item(0);
					node3.setTextContent(szul);
				}
			}
		}
